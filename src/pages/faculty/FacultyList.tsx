...
const FacultyList: React.FC = () => {
  const { faculty, deleteFaculty } = useCampusData();
  const { user } = useAuth();
              ...
              {currentItems.length > 0 ? (
                currentItems.map((faculty: Faculty) => (
                  <tr key={faculty._id} className="hover:bg-gray-50">
                    ...
                    <td className="whitespace-nowrap px-6 py-4 text-sm text-gray-500">{faculty.nidn}</td>
                    <td className="whitespace-nowrap px-6 py-4 text-sm text-gray-500">{faculty.fields.join(', ')}</td>
                    <td className="whitespace-nowrap px-6 py-4 text-sm text-gray-500">AKTIF</td>
                    <td className="whitespace-nowrap px-6 py-4 text-sm text-gray-500">{faculty.email}</td>
                    <td className="whitespace-nowrap px-6 py-4 text-center text-sm font-medium">
                      <div className="flex justify-center space-x-3">
                        <Link to={`/faculty/${faculty._id}`} className="text-blue-600 hover:text-blue-900" title="Lihat Detail">
                          <Eye size={18} />
                        </Link>
                        {user?.role === 'admin' && (
                          <>
