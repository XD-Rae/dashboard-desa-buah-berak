...
import { ArrowLeft, Edit, Trash2, Mail, Phone, MapPin, Book, Award, GraduationCap, FileText, Users, Calendar, Link as LinkIcon } from 'lucide-react';
...
const FacultyDetail: React.FC = () => {
  ...
  if (error || !faculty) {
    return (
      <div className="text-center py-12">
        <h2 className="text-xl font-semibold text-gray-900">Aparatur tidak ditemukan</h2>
        <p className="mt-2 text-gray-600">{error || 'Data Aparatur dengan ID tersebut tidak ditemukan.'}</p>
        <Link to="/faculty" className="mt-4 inline-flex items-center text-blue-600 hover:text-blue-800">
  ...
  const handleDelete = async () => {
    if (window.confirm(`Anda yakin ingin menghapus data aparatur ${faculty.name}?`)) {
      try {
        ...
        <div className="md:col-span-2">
          <div className="rounded-lg bg-white shadow-md">
            <div className="border-b border-gray-200 px-6 py-4">
              <h2 className="text-lg font-medium text-gray-900">Informasi Aparatur</h2>
            </div>
            <div className="p-6">
              ...
              <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                <div>
                  ...
                  <p className="text-gray-900">{faculty.nidn}</p>
                </div>
                <div>
                  <p className="text-sm font-medium text-gray-500">Jabatan</p>
                  <p className="text-gray-900">{faculty.fields.join(', ')}</p>
                </div>
                <div>
              ...
              <div className="mt-6">
                <h3 className="text-lg font-medium text-gray-900 mb-4">Lain-lain</h3>
                  ...
                  <div className="space-y-4">
                    {faculty.education.map((edu: any, index: number) => (
                      <div key={index} className="rounded-md bg-gray-50 p-4">
                        <div className="flex items-center">
                          <p className="text-sm font-medium text-gray-500">Jenjang Pendidikan :  </p>
                        ...
                        <div className="flex items-center">
                          ...
                          <GraduationCap  className="h-5 w-5 text-blue-600" />
                          <span className="ml-2 font-medium text-gray-900">{edu.year}</span>
                        </div>
                        <div className="flex items-center">
                          <p className="text-sm font-medium text-gray-500">Agama      :  </p>
                          <GraduationCap className="h-5 w-5 text-blue-600" />
                          <span className="ml-2 font-medium text-gray-900">{edu.institution}</span>
                        </div>
                        <div className="flex items-center">
                          <p className="text-sm font-medium text-gray-500">Tempat, Tanggal Lahir  :  </p>
                          <GraduationCap className="h-5 w-5 text-blue-600" />
                          <span className="ml-2 font-medium text-gray-900">{edu.field}</span>
                        </div>
                      </div>
                    ))}
                  </div>
                ...
                <div className="mt-6">
                  ...
                  {activeTab === 'publications' && (
                    <div>
                      ...
                      {faculty.publications && faculty.publications.length > 0 ? (
                        ...
                        <p className="text-gray-500">Tidak ada data publikasi</p>
                      )}
                    </div>
                  )}
                  ...
                  {activeTab === 'hki' && (
                    <div>
                      ...
                      {faculty.hki && faculty.hki.length > 0 ? (
                        <div className="space-y-4">
                          {faculty.hki.map((item: any, index: number) => (
                            <div key={index} className="rounded-md border border-gray-200 p-4">
                              <div className="flex items-start">
                                <Award className="h-5 w-5 text-indigo-600 mt-1 flex-shrink-0" />
                                <div className="ml-3">
                                  <p className="font-medium text-gray-900">{item.judulHKI}</p>
                                  <div className="mt-1 space-y-1 text-sm text-gray-600">
                                    <p>Nomor: {item.nomorHKI}</p>
                                    <p>Tanggal: {item.tanggalHKI}</p>
                                  </div>
                                  {item.linkDriveHKI && (
                                    <a
                        ...
                        <p className="text-gray-500">Tidak ada data HKI</p>
                      )}
                    </div>
                  )}
                  ...
                  {activeTab === 'pengabdian' && (
                    <div>
                      ...
                      {faculty.pengabdian && faculty.pengabdian.length > 0 ? (
                        <div className="space-y-6">
                          {faculty.pengabdian.map((item: any, index: number) => (
                            <div key={index} className="rounded-md border border-gray-200 p-4">
                              <div className="flex items-start">
                                <Users className="h-5 w-5 text-indigo-600 mt-1 flex-shrink-0" />
                                <div className="ml-3 flex-1">
                                  <p className="font-medium text-gray-900">{item.judulPengabdian}</p>
                                  ...
                                  {item.gambarPengabdian && item.gambarPengabdian.length > 0 && (
                                    <div className="mt-4 grid grid-cols-2 gap-2 sm:grid-cols-3 lg:grid-cols-4">
                                      {item.gambarPengabdian.map((img: string, imgIndex: number) => (
                                        <img
                                          key={imgIndex}
                                          src={img}
                        ...
                        <p className="text-gray-500">Tidak ada data pengabdian masyarakat</p>
                      )}
                    </div>
                  )}
                </div>
            ...
            <div className="p-6">
              <div className="mb-4">
                <div className="flex items-center">
                  ...
                  <p className="ml-2 text-sm font-medium text-gray-500">Email</p>
                </div>
                <p className="mt-1 text-gray-900">{faculty.email}</p>
              </div>
              ...
              {faculty.phone && (
                <div className="mb-4">
                  <div className="flex items-center">
                    ...
                    <p className="ml-2 text-sm font-medium text-gray-500">Telepon</p>
                  </div>
                  <p className="mt-1 text-gray-900">{faculty.phone}</p>
                </div>
              )}
              ...
              {faculty.address && (
                <div>
                  <div className="flex items-center">
                    ...
                    <p className="ml-2 text-sm font-medium text-gray-500">Alamat</p>
                  </div>
                  <p className="mt-1 text-gray-900">{faculty.address}</p>
                </div>
              )}
            </div>
          </div>
