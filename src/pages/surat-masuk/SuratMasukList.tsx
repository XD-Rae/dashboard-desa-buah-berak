...
import {SURAT_MASUK} from "../../types";

const SuratMasukList: React.FC = () => {
  const {suratMasuk, deleteSuratMasuk} = useDataContext();
  ...
  const handleDownload = async (url: string) => {
    try {
      ...
      const fileName = url.split("/").pop() || "dokumen.pdf";
  ...
  const filteredData = (suratMasuk || []).filter((s: SURAT_MASUK) => {
  ...
  return (
    <div>
      ...
      <div className="mb-6 flex flex-col md:flex-row md:items-center md:justify-between">
        <h1 className="text-2xl font-bold text-gray-900">Data Surat Masuk</h1>
        <Link
          to="/surat-masuk/new"
          ...
          <table className="min-w-full divide-y divide-gray-200">
            ...
            <tbody className="divide-y divide-gray-200 bg-white">
              {currentItems.length > 0 ? (
                currentItems.map((s: SURAT_MASUK) => (
                  <tr key={s.IDSURATMASUK} className="hover:bg-gray-50">
                    <td className="px-6 py-4 text-sm font-medium text-gray-900">
                      {s.NO_SURAT}
                    </td>
                    <td className="px-6 py-4 text-sm">{s.PENGIRIM}</td>
                    ...
                    <td className="px-6 py-4 text-center text-sm">
                      <div className="flex justify-center space-x-3">
                        {/* Tombol Preview Baru */}
                        <button
                          onClick={() => setPreviewFile(s.FILE_PATH)}
      ...
      {/* --- MODAL PREVIEW SURAT --- */}
      {previewFile && (
        <div className="fixed inset-0 z-[99] bg-black/60 flex items-center justify-center p-4 backdrop-blur-sm">
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-5xl h-[90vh] flex flex-col overflow-hidden">
            {/* Modal Header */}
            <div className="flex justify-between items-center px-6 py-4 border-b bg-gray-50">
              <h2 className="font-bold text-gray-800">Preview Dokumen</h2>
              <button
                onClick={() => setPreviewFile(null)}
                className="p-1 rounded-full hover:bg-gray-200 transition-colors text-gray-500 text-xl"
              >
                ✕
              </button>
            </div>
            ...
            <div className="flex justify-end gap-3 px-6 py-4 border-t bg-gray-50">
              <button
                ...
                className="inline-flex items-center px-4 py-2 bg-green-600 text-white rounded-md hover:bg-green-700 text-sm font-medium shadow-sm transition-colors"
              >
