...
import {ArrowLeft, Edit, Trash2, Mail, FileText, Calendar} from "lucide-react";
import toast from "react-hot-toast";

const SuratMasukDetail: React.FC = () => {
  const {id} = useParams<{id: string}>();
  const navigate = useNavigate();
  const {getSuratMasukById, deleteSuratMasuk} = useDataContext();
  ...
  const handleDelete = async () => {
    if (window.confirm(`Anda yakin ingin menghapus surat ini?`)) {
      try {
        await deleteSuratMasuk(surat._id);
    ...
    <div className="max-w-4xl mx-auto">
      <div className="mb-6 flex items-center justify-between">
        ...
        <div className="flex space-x-2">
          <Link
            to={`/surat-masuk/edit/${surat.IDSURATMASUK}`}
        ...
        <div className="p-6 space-y-6">
          ...
          <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
            <div>
              ...
              <p className="mt-1 text-gray-900">{surat.PENGIRIM}</p>
            </div>
